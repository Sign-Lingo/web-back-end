...
router.post("/register", userValidationRules(), validate, (req, res) => {
  ...
  User.add(user)
    .then((saved) => {
      ...
      level2.addUser(saved.id);
      level3.addUser(saved.id);
      level4.addUser(saved.id);
      level5.addUser(saved.id);
  ...
  User.findBy({ email })
    ...
    .then((user) => {
      console.log(user)
        ...
        // console.log(user_id)
        if (!user_id) {
          level1.addUser(user.id);
          level2.addUser(user.id);
          level3.addUser(user.id);
          level4.addUser(user.id);
          level5.addUser(user.id);
        }
